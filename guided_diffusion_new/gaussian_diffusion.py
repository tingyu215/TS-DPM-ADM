...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.alphas_cumprod_all = np.cumprod(alphas_all, axis=0)
        self.alphas_cumprod_prev_all = np.append(1.0, self.alphas_cumprod_all[:-1])
        self.alphas_cumprod_next_all = np.append(self.alphas_cumprod_all[1:], 0.0)
    ...
    def ddim_sample(
        ...
        xs.append(sample)
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "xs": xs}

    def get_next_time_window(self, current_time, window, xs, cutoff=300):
        window_shift = window // 2
        current_timestep = current_time[0]
        current_timestep = int(self.timestep_map[current_timestep])
        if current_timestep-window_shift > 0:
            next_time_list = self.all_timesteps[current_timestep-window_shift: current_timestep+window_shift+1]
        else:
        ...
        var = var.reshape(-1,1)
        var = var.expand_as(next_time_list)

        dist = (var-next_time_list)**2
        next_t_ = th.argmin(dist, dim=1)

        if current_timestep-window_shift > 0:
            n_t = next_t_ + current_timestep-window_shift
        else:
            n_t = next_t_
        
        if cutoff == 0:
            next_t = th.tensor(current_timestep, dtype=n_t.dtype, device=n_t.device).expand_as(n_t)
        elif len(xs) >1 and current_timestep > cutoff:
            next_t = n_t
        else:
            next_t = th.tensor(current_timestep, dtype=n_t.dtype, device=n_t.device).expand_as(n_t)
        # ipdb.set_trace()
