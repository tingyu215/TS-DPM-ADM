...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, all_betas, window, cutoff, do_time_shift, use_timesteps, **kwargs):
        self.use_timesteps = set(use_timesteps)
        self.timestep_map = []
        self.original_num_steps = len(kwargs["betas"])

        # self.all_timesteps = []
        base_diffusion = GaussianDiffusion( all_betas=all_betas, window=window, cutoff=cutoff, do_time_shift=do_time_shift,  **kwargs)  # pylint: disable=missing-kwoa
        last_alpha_cumprod = 1.0
        new_betas = []
        for i, alpha_cumprod in enumerate(base_diffusion.alphas_cumprod):
            # self.all_timesteps.append(i)
